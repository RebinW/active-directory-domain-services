...
In Active Directory, reverse lookup zones are considered a best practice because they allow admins and applications to easily identify which host maps to a given IP address.
...
When installing the DNS server role on a Windows server, by default a reverse lookup zone is not created. Microsoft recommonds and says that best practice to is to have a reverse lookup zone and have PTR records for all internal resources we manage. Reverse lookups are performed using PTR records.
...
In server manager there is already a default *Reverse Lookup Zones* folder, we need to create the zone itself by right clicking on the folder and choose *New Zone*:
...
- Condition 1: A reverse lookup zone exists "We just created one"
...
To make it crystal clear, dynamic DNS updates are of course directly related to DHCP. When a client recieves an IP address from a DHCP server, it attempts to register its DNS records, this happens for both A records and PTR records automatically. If the client later recieves an new IP address, it updates the records.
- Old PTR record is automatically removed.
- New PTR record is automatically created.

Who actually updates the DNS information? DNS updates can be enabled and handled by the client ifself, but since DHCP is the one assigning IP addresses it is best practice to configure DHCP to handle this. DHCP keeps track, when lease expires or changes, DHCP updates or removes the records.
...
- Host name: you can manually write the hostname, but it is best practice to choose the corresponding recourse in the browser menu, and locate its A record in the forward lookup zone, it will automatically fill the field with dc01.klarstroem.local

In later module, when I join clients to the domain I will also show the automatic creation of A, AAAA, and PTR records.

## Results
The reverse lookup zone successfully allows IP addresses to be resolved to hostnames. PTR records stored in the zone enables admins and systems/applications to identify hostsnames when only IP addresses are known.
...
## Lessons Learned
This lab demonstrated how reverse lookup zones complement forward lookup zones by enabling IP to hostnames resolution. Reverse DNS is particularly useful when analyzing logs, monitoring systems, or troubleshooting network activity.
