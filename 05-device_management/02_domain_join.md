...
In the previous lab I set up the VM and installed Windows 11 Pro. I configured the network settings ensuring the machine had an static IP within the internal network "Host-Only adapter". The DNS server was also configured to point to the domain controller to allow name resolution and domain discovery. 
...
#### Step 3. Logon using domain user
After reboot, the login screen allowed authentication using domain credentials. I now logged on to the client using the domain user Mark Nielsen:
![Logon](screenshots/loginmark.png)

#### Step 4. Relocating client to the correct OU
On the domain controller, the device by default appears in the "Computers" container. The device was moved to the "Workstations" organizational unit to ensure structure and policy enforcement:
![Moving the client](screenshots/moveclient.png)

## Verification
The client domain-join was verified by logging into the client using the domain user mark.nielsen@klarstroem.local. Successful login confirmed that authentication was handled by Active Directory.
